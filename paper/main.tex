...
\graphicspath{{../reports/figures/}{../reports/early_warning/figures/}{../reports/virtual_metrology/figures/}{../reports/attribution/figures/}}
...
Public Virtual Metrology, Physics, Coupling, Early Warning, Attribution, and
Frozen Supervisory Design}
...
evidence, the frozen WP12 simulation-only early-warning experiment, and WP13
conditional synthetic root-cause classification. The PHM target unit is
source-undeclared and the selected tree's primary intervals undercover. WP12
has only three independent held-out events and fails under important shifts.
WP13 uses a diagnostic positive-warning token, and delay/dropout collapse its
availability. WP15/WP16 freeze a hold/resume controller and independent
synthetic safety contract, but implementation and efficacy remain unavailable.
No
...
efficacy, real-tool calibration, or topology-independent applicability. A
separately frozen diagnostic study compares always-unknown, residual-rule,
multinomial-logistic, and hybrid root-cause estimators on 66 held-out whole
runs. The preregistered hybrid attains 0.8636 accuracy and macro recall, 1.0000
unknown recall and selective accuracy, and 0.8500 known-cause coverage; all
nine errors are abstentions. Rule only scores higher at 0.9242, but is not
substituted after TEST access. Severe delay and dropout collapse diagnostic
availability toward unknown. This is simulator-conditional classification,
not experimental causal proof or permission to act. A controller-authority
audit finds that already-maximal utility commands and reduction-only CMP
actions cannot defensibly correct the primary under-removal mechanism. The
frozen predictive action set is therefore advisory, safe hold, and controlled
resume with explicit recipe-phase restoration. A separate safety contract
checks arrived-sensor validity, uncertainty/applicability, magnitude, slew,
hold, and restart conditions. In a CMP-only development limiting case,
warning-timed hold reduces peak relative MRR deviation from 5.526% to 3.895%
versus no action, while an upstream utility threshold reaches 0.087% with a
substantially longer hold. This establishes a feasible intervention direction
and a strong mandatory comparator, not integrated controller efficacy or
equipment safety.
\end{abstract}
...
early warning; root-cause attribution; supervisory control; sensitivity
analysis; simulation
...
an open-loop early-warning experiment with disjoint run-level
fitting/calibration roles, and conditional synthetic root-cause attribution
with mandatory abstention. The predictive-controller objective, action
authority, phase clock, baselines, independent safety constraints, and paired
success gates are now frozen. Their Phase 4 implementation and the integrated
comparison remain protocols until acceptance evidence exists.
...
\begin{enumerate}
  ...
  and
  \item a frozen ten-cause-plus-unknown diagnostic classifier comparison with
  arrived-only features, mechanistic residuals, compound-event abstention,
  held-out class metrics, and communication-corruption diagnostics.
\end{enumerate}
...
preregistered simulator excursion target on the named primary ensemble. A
frozen hybrid also classifies initiating simulator labels conditionally on a
valid positive warning and abstains under declared uncertainty gates. The
...
shifts; the attribution claim uses only six runs per class and loses
availability under delay and dropout. The study does not claim prediction or
prevention of physical defects, real wafer-yield loss, real equipment damage,
controller benefit, or readiness for production control. Feature contributions
and rule chains are not described as causal proof.
...
\section{Virtual metrology, early warning, and attribution}

WP09 is a frozen offline public-data result, WP12 a frozen synthetic
early-warning result, and WP13 a conditional synthetic attribution result.
Control, safety, and integrated-runtime subsections remain protocols. The
WP12/WP13 runners are open loop: they do not propose or apply controller
actions and are not the WP17 runtime.
...
frequency deviation, pump trip, valve restriction, demand spike, thermal
excursion, pressure-sensor fault, and flow-sensor fault. \textsc{Unknown} is a
required eleventh output. UPS transfer and VFD derating are recorded only as
propagation evidence. Simulator labels are joined after estimation for offline
scoring and are absent from the online window
\begin{equation}
 {\cal O}(t_d)=\{o_i:t_{\mathrm{arrival},i}\leq t_d\}.
 \label{eq:attribution-window}
\end{equation}
The estimator computes latest value, 0.75 s mean, least-squares slope, and
missing fraction for 14 observed utility and automation signals. It excludes
future records, latent state, scenario metadata, CMP MRR, pad and coupling
state, and offline excursion or cause labels.
...
\begin{aligned}
 ...
 \widehat Q_t&=\min[D,v\max(P_{\mathrm{obs}}-P_r,0)/R_t],\\
 r_Q&=(Q_{\mathrm{obs}}-\widehat Q_t)/Q_{t,0}.
\end{aligned}
\label{eq:attribution-residuals}
\end{equation}
These are engineering diagnostic proxies, not independent hydraulic truth.
Sensor-fault scenarios corrupt observations without changing latent hydraulics.
Rule severities use
\begin{equation}
 \rho(d;a,b)=\operatorname{clip}\left((d-a)/(b-a),0,1\right).
 \label{eq:attribution-ramp}
\end{equation}

A balanced multinomial logistic comparator uses TRAIN-only median imputation
and standardization and a probability temperature selected on disjoint
CALIBRATION runs. The signed contribution
$a_{cj}=\beta_{cj}\widetilde x_j$ explains a fitted logit only; it is not a
causal effect. The preregistered primary method pools rule and learned
probabilities geometrically:
\begin{equation}
\begin{aligned}
 s_c={}&0.5\log(p_{\mathrm{ML},c}+10^{-9})\\
      &+0.5\log(p_{\mathrm{rule},c}+10^{-9}),\\
 p_{\mathrm{hyb}}={}&\operatorname{softmax}(s).
\end{aligned}
\label{eq:attribution-fusion}
\end{equation}
Negative or uncertain warnings, stale or missing sensors, out-of-distribution
features, low confidence or margin, rule/model conflict, and multiple strong
initiating chains force \textsc{unknown}. The primary diagnostic study supplies
a neutral valid-positive singleton warning to isolate cause separability from
WP12 recall; it is not an end-to-end operational rate.

TEST contains 66 disjoint whole runs: six for each known cause and six normal
\textsc{unknown} cases. Table~\ref{tab:wp13-primary} and
Fig.~\ref{fig:wp13-results} report the frozen result. The hybrid correctly
classifies 57/66 runs. All nine errors are conservative abstentions; it never
substitutes an incorrect known cause. Pressure-sensor-fault recall is weakest
at 0.3333. The rule-only comparator scores higher than the primary hybrid, but
post-TEST model switching is prohibited.

\begin{table*}[t]
\centering
\caption{Held-out WP13 whole-run attribution. Coverage is the fraction of
known-cause runs receiving a known label; selective accuracy conditions on
such labels. All quantities are conditional synthetic diagnostic results.}
\label{tab:wp13-primary}
\small
\begin{tabular}{lrrrrrr}
\toprule
Method & Accuracy & Macro recall & Unknown recall & Known coverage &
Selective accuracy & Top-2\\
\midrule
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.54\textwidth]{wp13_hybrid_confusion.png}\hfill
\includegraphics[width=0.43\textwidth]{wp13_hybrid_recall.png}
\caption{Frozen WP13 hybrid result. Left: run-level confusion matrix; every
off-diagonal count is an abstention to \textsc{unknown}. Right: held-out recall
by initiating simulator label. These are synthetic classifications, not
experimental causal identification.}
\label{fig:wp13-results}
\end{figure*}

Grouped-bootstrap hybrid accuracy has 5th--95th percentiles
0.7879--0.9242. On six unseen interruption/demand compounds, the estimator
abstains on 5/6 and its pre-abstention top two contain both offline truth labels
on 6/6. Doubled noise and parameter mismatch retain 0.8636 aggregate accuracy.
In contrast, 0.20 s delay forces all runs to \textsc{unknown}, and 10\% packet
loss reduces known-cause coverage to 0.15 while selective accuracy remains
1.0. These are diagnostic-availability failures rather than false known-cause
substitutions. Hybrid local inference has median/95th-percentile times of
3.423/3.746 ms; these are not production-latency guarantees.

\subsection{WP14--WP16: baselines, predictive supervision, and safety}

\textbf{Frozen protocol; runtime result pending.} The authority audit narrows
the first predictive policy. The primary disturbance removes conditioning
support during \textsc{dress}, stores a pad-activity deficit, and causes later
under-removal. VFD and valve commands are already at their maxima. The
remaining canonical CMP actions are reductions, which cannot be represented as
a validated correction for under-removal under positive Preston exponents.
The primary action set is therefore
\[
\begin{aligned}
\mathcal A_P=\{&\textsc{no action},\ \textsc{advisory},\\
               &\textsc{safe hold},\ \textsc{controlled resume}\}.
\end{aligned}
\]
Numerical actions retain independent bounds but are disabled. Unvalidated
upward compensation is not introduced to make control appear effective.

The controller binds to the frozen WP12 logistic artifact, exact
feature/configuration identities, the \textsc{dressing-water-support}
topology, a 3.0 s horizon, and a 0.10 s decision period. For calibrated risk
$p_k$, conformal set $S_k$, valid uncertainty $u_k$, and applicable
configuration $a_k$, hold requires
\begin{equation}
\begin{aligned}
g_k^+={}&\mathbf 1[p_k\ge0.50]\mathbf 1[S_k=\{1\}]\\
        &{}\times\mathbf 1[u_k=\mathrm{valid}]
        \mathbf 1[a_k=\mathrm{applicable}]=1.
\end{aligned}
\label{eq:control-gate}
\end{equation}
An otherwise valid $p_k\ge0.30$ emits an advisory only. Resume requires
$p_k\le0.20$, $S_k=\{0\}$, and the independent release contract. Attribution
is explanatory only and \textsc{unknown} is never interpreted as normal.

The controller states are \textsc{running}, \textsc{holding},
\textsc{recovering}, and \textsc{complete}. Wall time and recipe time are
separate:
\begin{equation}
\dot\tau_{\mathrm{recipe}}=
\begin{cases}
1,&z=\textsc{running},\\
0,&z\in\{\textsc{holding},\textsc{recovering}\}.
\end{cases}
\label{eq:recipe-clock}
\end{equation}
An interrupted \textsc{dress} is restored through
\textsc{hold}$\rightarrow$\textsc{recover}$\rightarrow$\textsc{prepare}
$\rightarrow$\textsc{dress} and completes its remaining work. A bounded
lexicographic tie break uses
\begin{equation}
J_k(a)=8p_kr(a)+\frac{\Delta T_{\mathrm{phase}}}{3.0\ \mathrm{s}}
+\frac{\Delta T_{\mathrm{hold}}}{3.0\ \mathrm{s}}
+0.1\mathbf 1[a\ne a_{k-1}],
\label{eq:control-score}
\end{equation}
subject to hard probability and safety gates. These synthetic weights are not
an optimal-control result.

Every paired run completes the same recipe scope or is a failed completion at
the frozen 10 s extension. Comparisons align by active-polish progress $\tau$:
\begin{align}
E_{\mathrm{IAE},c}&=\int_0^{\tau_f}
|R_c(\tau)-R_{\mathrm{ref}}(\tau)|\,d\tau,\\
E_{\mathrm{peak},c}&=\max_{\tau\in[0,\tau_f]}
|R_c(\tau)-R_{\mathrm{ref}}(\tau)|.\label{eq:control-errors}
\end{align}
Cumulative-removal error, hold, recovery, cycle delay, and action effort are
mandatory. Thus stopping the recipe cannot shorten the evaluation scope.

The primary fixed threshold consumes arrived simulated MRR in
\textsc{polish}, holds after 0.25 s outside $[0.95,1.05]$ of the paired
phase-progress reference, and releases inside $[0.97,1.03]$. A stronger,
mandatory upstream utility comparator holds after 0.10 s outside
$[0.90,1.10]$ or a 2 K thermal band and releases after 1.0 s inside
$[0.95,1.05]$ and 1 K. Neither comparator sees predictions, attribution,
latent state, or event truth.

Table~\ref{tab:hold-feasibility} reports a pre-implementation limiting case
using the WP08 CMP equations, imposed conditioning loss over 0--5.99 s, and
equal completed \textsc{dress}/\textsc{polish}. The warning time is the 7.00 s
excursion onset minus WP12's 2.71 s median lead.

\begin{table*}[t]
\centering
\caption{CMP-only hold feasibility. No streaming predictor, controller,
sensor, safety filter, or robustness runtime is included.}
\label{tab:hold-feasibility}
\small
\begin{tabular}{lrrrr}
\toprule
Development case & Peak relative MRR error & Mean MRR ratio & Hold (s) &
Cycle extension (s)\\
\midrule
Disturbed no action & 5.526\% & 0.944762 & 0.00 & 0.00\\
Warning-timed hold, 4.29 s & 3.895\% & 0.961057 & 1.70 & 2.20\\
Utility-threshold hold, 0.10 s & 0.087\% & 0.999126 & 5.89 & 6.39\\
\bottomrule
\end{tabular}
\end{table*}

Warning-timed hold has a feasible direction versus no action, but direct
utility thresholding is far stronger on MRR and more expensive in hold time.
The latter must remain visible even if it defeats a predictive-benefit claim.

The WP16 filter is separately loaded and forbidden from importing a
controller. For the latest arrived required observation $o_s^*$,
\begin{equation}
\begin{aligned}
\operatorname{fresh}_s(k)={}&
\mathbf 1[o_s^*\ \mathrm{exists}]
\mathbf 1[t_{\mathrm{arrival},s}\le t_k]\\
&{}\times\mathbf 1[0\le t_k-t_{\mathrm{source},s}\le A_s].
\end{aligned}
\label{eq:safety-freshness}
\end{equation}
It checks schema/timing, blocking quality flags, signal consistency, model and
topology binding, uncertainty/applicability, mode, magnitude, slew, hold, and
restart. Exactly one final action receives \textsc{approved},
\textsc{clipped}, \textsc{rejected-out-of-envelope},
\textsc{rejected-sensor-invalid}, \textsc{rejected-high-uncertainty}, or
\textsc{replaced-with-hold}. Invalid sensing never authorizes actuation or
resume; hold remains reachable.

Automatic utility holds apply only in \textsc{prepare}/\textsc{polish}.
Conditioning loss during \textsc{dress} is a simulated quality-risk mechanism,
not a validated equipment hazard. Release requires valid utilities in
$[0.95,1.05]$, temperature within 1 K for 1.0 s, warning clear for 0.50 s,
minimum hold 0.50 s, recovery dwell 0.50 s, and synthetic battery reserve
\[
E_{\min}=1.25\frac{2500\ \mathrm{W}}{0.95}(3.0+0.5)\ \mathrm{s}
=11513.16\ \mathrm{J}.
\]
For constraint set $\mathcal C_k$, the final software invariant is
\begin{equation}
\forall k:\quad a_k^F\in\mathcal A_{\mathrm{canonical}}
\land a_k^F\in\mathcal C_k
\land t_{\mathrm{effective}}(a_k^F)\ge t_{k+1}.
\label{eq:final-action}
\end{equation}
The 21-check machine-readable audit and 13 focused tests validate the frozen
contracts, not runtime enforcement, functional safety, or equipment limits.

\subsection{WP17--WP18: integrated paired evaluation}

\pending{}: no action, process-MRR threshold, and predictive control will be
run on identical scenarios, initial states, parameter draws, seed maps, and
sensor corruptions; the mandatory utility-threshold comparator is reported
beside them. A manifest equality assertion will differ only in controller
identity and the trajectories subsequently caused by its actions. Controller
development, primary TEST, and robustness seed ranges begin at 130000, 230000,
and 330000, and no WP12/WP13 seed may enter controller TEST. The final
comparison will include normal and healthy-sag negative controls, hold-required
grid interruption, pump trip, valve restriction, demand spike, temperature
event, sensor bias/dropout, slow drift, parameter mismatch, and compound unseen
cases.
...
safety-filter rejections, attribution accuracy, prediction/controller/filter
latency, and simulation throughput. Summaries will report mean, median,
standard deviation, 5th and 95th percentiles, and worst case. A predictive-
benefit claim requires positive median peak and integrated MRR-error improvement
versus no action with non-negative run-bootstrap lower bounds. Versus the
process-MRR threshold, one MRR metric must improve at least 5\% while the other
and cumulative-removal error worsen no more than 5\%. At least 80\% of hold-
required pairs must be non-worse than no action on both MRR metrics, negative-
control hold rate must not exceed 5\%, equal recipe completion is required, and
final constraint violations must be zero. The utility-threshold result remains
visible regardless of outcome. No such efficacy claim is made here.

\begin{table*}[t]
\centering
\caption{Manuscript evidence status at the Phase 3 scientific freeze.
``Protocol'' means the method is frozen but no runtime performance result is
asserted.}
\label{tab:status}
\small
...
WP13 & Attribution & Synthetic result & Arrived-only evidence, abstention,
whole-run class and robustness results\\
WP14 & Baseline controllers & Protocol & No-action, process-MRR threshold, and mandatory utility-threshold specifications\\
WP15--WP16 & Predictive control and safety & Frozen contract & Authority audit, hold/resume policy, recipe clock, independent constraints, and paired gates; no runtime result\\
...
WP13 adds a second distinction: diagnostic correctness versus diagnostic
availability. On the primary grid the hybrid never replaces one known cause
with another, but it abstains on nine known-cause runs. This produces perfect
selective accuracy at 0.85 known-cause coverage. Pressure-sensor faults expose
the limited independent redundancy of the reduced-order plant, while delay and
dropout drive the estimator toward \textsc{unknown}. A runtime must not treat
\textsc{unknown} as evidence that the process is normal. The rule-only
comparator also exceeds the hybrid on held-out accuracy, showing that learned
fusion has not demonstrated incremental value in this ensemble. Preserving the
frozen primary avoids post-hoc selection; it does not hide the stronger
comparator. Rule chains, residual agreement, and coefficient contributions
remain diagnostic explanations within the simulator rather than causal proof.
...
The control-authority audit also prevents a common modelling shortcut. Because
the utility commands are already maximal and the allowed recipe changes are
reductions, direct numerical compensation would either be ineffective for the
under-removal pathway or require inventing unsupported authority. Hold and
phase restoration are physically narrower and more auditable, but their cost
is explicit cycle delay. The limiting case shows a feasible warning-timed
direction versus no action while also showing that direct utility thresholding
can dominate MRR protection at greater hold cost. The latter is a useful
negative design result, not a comparator to suppress.
...
rejection, clipping, hold fallback, and restart tests. Its constraints, battery
reserve, and dwell times are synthetic study assumptions rather than certified
tool limits. Runtime enforcement and controller efficacy remain future work.
...
\begin{itemize}
  ...
  \item WP13 primary scoring supplies a neutral valid-positive warning, so its
  accuracy is conditional cause separability rather than an end-to-end
  warning-and-diagnosis rate.
  \item WP13 has only six TEST runs per class on a fixed synthetic severity
  grid. The grouped bootstrap resamples these runs but cannot create missing
  mechanisms or real-fab diversity.
  \item Rule only outperforms the frozen hybrid on TEST, so learned fusion has
  not established incremental value. The primary method is retained to avoid
  post-TEST selection, not because it is empirically superior.
  \item Pressure-sensor-fault recall is 0.3333. A 0.20 s communication delay
  forces all diagnoses to \textsc{unknown}, and 10\% packet loss reduces
  known-cause coverage to 0.15. Diagnostic unavailability must not be treated
  as a confirmed normal state.
  \item The primary predictive policy has hold/resume authority only. Numeric
  utility and CMP actions are disabled because they cannot defensibly correct
  the validated under-removal mechanism under the current plant constraints.
  \item The hold-feasibility result imposes a median-lead-derived action time
  and bypasses streaming prediction, sensor delivery, controller state, and
  safety filtering. It is not a closed-loop efficacy result.
  \item The independent constraint values, battery reserve, validity ages,
  dwell times, and latency budgets are synthetic software-study assumptions,
  not equipment ratings or a functional-safety assessment.
  \item Public-data virtual metrology is an offline source-native result only.
  Controller, safety-filter, integrated-runtime, and controller-robustness
  results are unavailable at this evidence freeze.
...
WP13 was opened once from clean Git commit
\texttt{ec7bc8b41a\ldots 15a9540c} after 23 focused and 211 complete tests
passed with warnings treated as errors. Its persistent opening-marker hash is
\texttt{53139b8b\ldots 502a3d}; the 198-row, 66-run TEST payload hash is
\texttt{395464f2\ldots 24329}. Validation JSON, deterministic payload, and
prediction CSV hashes are \texttt{f075e513\ldots 13fefc},
\texttt{b1f7080d\ldots e74f99}, and
\texttt{8108f514\ldots 494eb}. The confusion and recall figure hashes are
\texttt{07db0b4d\ldots 2b57ad} and
\texttt{add6b0cf\ldots 11615}. The persistent marker prevents a silent second
holdout execution.

WP15/WP16 scientific-contract verification passes 13 focused unit tests and
all 21 machine-readable cross-contract checks. Predictive, baseline, and
safety configuration hashes are \texttt{219849ca\ldots 0f265},
\texttt{d527490e\ldots 794ec}, and
\texttt{032bb745\ldots 707c}. Contract-validation and CMP-only feasibility
JSON hashes are \texttt{86e3ab72\ldots 3d7a} and
\texttt{170687de\ldots 136}. These artifacts freeze design and limiting-case
evidence only; they contain no controller TEST outcome.
...
are recorded in a separate figure manifest. The two WP13 figures are generated
by the one-shot attribution driver from the frozen held-out result. The
evidence map links each quantitative statement to the governing report and
artifact. The article is rebuilt locally with
...
changes automatically preserve the result. The analogous WP13 preflight counts
are recorded above. Reproducible paired controller manifests and the final
claims audit remain pending.
...
Conditional on a valid diagnostic warning, the frozen hybrid identifies 57/66
held-out initiating labels and abstains on every error; rule only identifies
61/66 and therefore remains an important stronger comparator. Communication
delay and dropout sharply reduce diagnostic availability, and none of these
simulator-label results constitutes experimental causal proof.
...
bounded control, independent safety filtering, and paired robust evaluation are
complete. Its scientific protocol is now explicit: hold/resume is the only
defensible primary authority, recipe progress freezes during hold/recovery,
the process-MRR threshold is the primary comparator, the upstream utility
threshold is mandatory, and every proposal passes an independent fail-closed
constraint contract. One limiting case supports a feasible direction versus
no action but also shows the utility threshold can be stronger. Supported
conclusions remain limited to the retained PHM roles, the tested synthetic
equations and declared topology, the named WP12/WP13 ensembles, and internal
consistency of the frozen WP15/WP16 design.
